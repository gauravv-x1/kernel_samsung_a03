...
export ARCH=arm64
export CLANG_TOOL_PATH=$(pwd)/toolchain/bin
# export CLANG_TOOL_PATH=/home/grahame/toolchains/clang/bin/
...
make -C $(pwd) O=$(pwd)/out BSP_BUILD_DT_OVERLAY=y
CC=clang 
LD=ld.lld 
ARCH=arm64
CLANG_TRIPLE=aarch64-linux-gnu- physwizz_defconfig -j12
make -C $(pwd) O=$(pwd)/out BSP_BUILD_DT_OVERLAY=y
CC=clang
LD=ld.lld
ARCH=arm64
